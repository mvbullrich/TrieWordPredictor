...
int main() {
  ...
  printf("String con todos los caracteres alfanuméricos: %d\n", strLen("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"));
  printf("\n");
  
  printf("strDup\n");
  printf("String vacio: %s\n", strDup(""));
  printf("String de un caracter: %s\n", strDup("s"));
  printf("String con todos los caracteres alfanuméricos: %s\n", strDup("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"));
  printf("\n");
  
  struct keysPredict* test1 = keysPredictNew();
    // COMPLETAR
    ...
    // keysPredict - encontrar palabras (1 es que encontró y 0 es que no enconrtró)
    findAndPrintAll(kt, "c");
    findAndPrintAll(kt, "cas");
    findAndPrintAll(kt, "oreja");
    findAndPrintAll(kt, "zazz");

    printf("\n");
    struct node* xx = keysPredictFind(kt, "p");
    if (xx != NULL) {
        printf("FIND: %s\n", xx->word); // Imprime la palabra del nodo encontrado
    } else {
        printf("FIND: es NULL\n"); // Indica que no se encontró la palabra
    }
    printf("\n");

//     // keysPredict - predecir palabras
    predictAndPrintAll(kt,"or");
    predictAndPrintAll(kt,"di");
    predictAndPrintAll(kt,"pre");
